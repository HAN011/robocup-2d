...
class _StrategyFormation:
    def __init__(self):
        ...
        project_base_dir = project_root / "base"
        vendor_dt_dir = Path(__file__).resolve().parent / "formation_dt"

        offense_path = vendor_dt_dir / "offense_formation.conf"
        defense_path = vendor_dt_dir / "defense_formation.conf"
        if not offense_path.exists():
            offense_path = project_base_dir / "formation_dt" / "offense_formation.conf"
        if not defense_path.exists():
            defense_path = project_base_dir / "formation_dt" / "defense_formation.conf"

        self._formations = {
            "normal": Formation(str((project_root / "formations" / "443.conf").resolve())),
            "offense": Formation(str(offense_path.resolve())),
            "defense": Formation(str(defense_path.resolve())),
            "before_kick_off": Formation(str((project_base_dir / "formation_dt" / "before_kick_off.conf").resolve())),
            "goalie_kick_our": Formation(str((project_base_dir / "formation_dt" / "goalie_kick_our_formation.conf").resolve())),
            "goalie_kick_opp": Formation(str((project_base_dir / "formation_dt" / "goalie_kick_opp_formation.conf").resolve())),
            "kickin_our": Formation(str((project_base_dir / "formation_dt" / "kickin_our_formation.conf").resolve())),
            "setplay_our": Formation(str((project_base_dir / "formation_dt" / "setplay_our_formation.conf").resolve())),
            "setplay_opp": Formation(str((project_base_dir / "formation_dt" / "setplay_opp_formation.conf").resolve())),
        }
    ...
    def update(self, wm):
        ...
        teammate_min = wm.intercept_table().teammate_reach_cycle()
        opponent_min = wm.intercept_table().opponent_reach_cycle()
        ball_cycle = min(self_min, teammate_min, opponent_min)
        ball_pos = wm.ball().inertia_point(ball_cycle) if wm.ball().pos_valid() else Vector2D(0, 0)
